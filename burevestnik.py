...
VIDEO_ANALYSIS_STEP_DEFAULT = 5
VIDEO_ANALYSIS_STEP_OPTIONS = ["1", "2", "3", "5", "10", "15"]
...
class BurevestnikSystem(ctk.CTk):
    def __init__(self):
        ...
        self.model_paths = self._find_model_files()
        self.model_labels = [path.name for path in self.model_paths]
        self.model_by_label = {path.name: path for path in self.model_paths}
        self.selected_model_path = self._select_initial_model_path()
        self.video_analysis_step = VIDEO_ANALYSIS_STEP_DEFAULT
        self.frame_number = 0
        self.last_analysis_frame_number = 0
        self.last_analysis_gap = 0
    ...
    def _find_model_files(self):
        paths = []
        for folder in (MODELS_DIR, PROJECT_ROOT):
        ...
        return sorted(paths, key=lambda path: (path.name != "best.pt", path.name.lower()))
    def _select_initial_model_path(self):
    ...
    def _model_label(self):
        return self.selected_model_path.name if self.selected_model_path else "best.pt"
    def _load_model(self):
        try:
            model_path = self.selected_model_path or (MODELS_DIR / "best.pt")
            self.status_label.configure(text=f"Загружаю модель {model_path.name}...")
            self.model = YOLO(str(model_path))
            self.status_label.configure(text=f"Модель {model_path.name} загружена. Готов к выбору источника.")
    ...
    def _build_layout(self):
        ...
        self.debug_frame = ctk.CTkFrame(self.sidebar, fg_color="#0f172a", corner_radius=12)
        self.debug_frame.grid(row=12, column=0, padx=24, pady=(0,14), sticky="ew")
        self.debug_frame.grid_columnconfigure(0, weight=1)
        debug_title = ctk.CTkLabel(self.debug_frame, text="Отладка", text_color="#e5e7eb", font=ctk.CTkFont(size=15, weight="bold"), anchor="w")
        debug_title.grid(row=0, column=0, padx=12, pady=(10,6), sticky="ew")
        self.model_menu = ctk.CTkOptionMenu(self.debug_frame, values=self.model_labels or ["best.pt"], command=self.change_debug_model)
        self.model_menu.grid(row=1, column=0, padx=12, pady=(0,8), sticky="ew")
        self.model_menu.set(self._model_label())
        step_row = ctk.CTkFrame(self.debug_frame, fg_color="transparent")
        step_row.grid(row=2, column=0, padx=12, pady=(0,8), sticky="ew")
        step_row.grid_columnconfigure(1, weight=1)
        step_caption = ctk.CTkLabel(step_row, text="Шаг видео", text_color="#9ca3af", anchor="w")
        step_caption.grid(row=0, column=0, padx=(0,8), sticky="w")
        self.analysis_step_menu = ctk.CTkOptionMenu(step_row, values=VIDEO_ANALYSIS_STEP_OPTIONS, width=90, command=self.change_analysis_step)
        self.analysis_step_menu.grid(row=0, column=1, sticky="e")
        self.analysis_step_menu.set(str(self.video_analysis_step))
        self.debug_info_label = ctk.CTkLabel(self.debug_frame, text="", text_color="#9ca3af", justify="left", anchor="w")
        self.debug_info_label.grid(row=3, column=0, padx=12, pady=(0,10), sticky="ew")
        self.status_label = ctk.CTkLabel(self.sidebar, text="Готов к выбору источника", justify="left", wraplength=290, text_color="#9ca3af")
        self.status_label.grid(row=13, column=0, padx=24, pady=(0,22), sticky="ew")
    ...
    def change_debug_model(self, label):
        path = self.model_by_label.get(label)
        if path is None:
            return
        ...
        self._load_model()
    def change_analysis_step(self, value):
        try:
            self.video_analysis_step = max(1, int(value))
        except ValueError:
            self.video_analysis_step = VIDEO_ANALYSIS_STEP_DEFAULT
        self._reset_analysis_stats()
        self._update_debug_panel()
    def _reset_analysis_stats(self):
        ...
        self.analysis_runs = 0
        self.last_detection_count = 0
    def _should_analyze_frame(self):
        if self.current_mode != "Тест на видео":
            return True
        return self.frame_number == 1 or self.frame_number % self.video_analysis_step == 0
    def _update_debug_panel(self):
        if not hasattr(self, "debug_info_label"):
            return
        mode_step = "каждый кадр" if self.current_mode != "Тест на видео" else f"каждый {self.video_analysis_step}-й кадр"
        fact = "-" if self.last_analysis_gap == 0 else f"{self.last_analysis_gap} кадр."
        text = (
            f"Модель: {self._model_label()}\n"
            f"Анализ видео: {mode_step}\n"
            f"Факт: {fact} | YOLO: {self.last_analysis_ms:.0f} мс"
        )
        self.debug_info_label.configure(text=text)
    ...
    def _read_next_frame(self):
        ...
        if self.model is not None and self.show_detections and should_analyze:
            analysis_started = time.perf_counter()
            processed = self._process_frame(frame)
            self.last_analysis_ms = (time.perf_counter() - analysis_started) * 1000
            if self.last_analysis_frame_number:
                self.last_analysis_gap = self.frame_number - self.last_analysis_frame_number
            else:
                self.last_analysis_gap = 1
            self.last_analysis_frame_number = self.frame_number
